...
| 项 | 影响 | 现状 |
|---|---|---|
| **低于本地合成下限的机型本地合成慢** | 低于下限的机型（骁龙 8 Gen 1／天玑 9300 **以下**，如 SD845 级实测单句合成可达 **1 分钟以上**；模型本身的算力开销）远慢于实时 | 属**已知限制**：本地模式会提示最低要求（RQ-515 修订后为**无条件告知**；**现行包仍按旧判定逻辑弹「性能不足，可能延迟极大」，待随下一批次改为无条件告知**），**最终决定权在用户**；此类设备按 RQ-516 **不判缺陷**；**在线合成不设算力下限**（RQ-519）；后续方向为轻量模型档位与结果复用 |
