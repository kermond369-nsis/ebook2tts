...
> 生成：2026-09-16 ｜ 实施线：Hermes ｜ 设备：**直接 USB 接入本服务器**（`adb -s [DEVICE-SERIAL]`）
> 定位：**性能回归锚定机**（取最差情况的性能下界）｜ 用途：P7-D 性能调优目标机
> ⚠️ **勘误（2026-09-21）**：本机（SD845）**低于本地合成下限**（RQ-519：骁龙 8 Gen 1+／天玑 9300+）。原文「低端锚定机（NFR §5.1『保底档』参照）」系**实施线自设口径、非甲方要求**，且与需求书原保底档（天玑 1000 级）冲突 —— 两者均已作废。本报告数据**仅作性能下界参照**，不代表机队下限；该机的适用场景是**在线合成**。
...
3. 补测 **VITS zh-ll** 在本机的 TTFT/RTF/内存（**注**：本机低于本地合成下限，结果只作下界参照，**不能用于保底档达标判定**）；
